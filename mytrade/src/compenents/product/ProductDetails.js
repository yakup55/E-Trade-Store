import {
  Alert,
  Avatar,
  Button,
  ButtonGroup,
  Chip,
  Grid,
  InputLabel,
  TextField,
  Typography,
} from "@mui/material";
import { Container, Stack } from "@mui/system";
import React, { useEffect } from "react";
import { useDispatch, useSelector } from "react-redux";
import { useNavigate, useParams } from "react-router-dom";
import { getOneProductWithDetail } from "../../redux/actions/productActions";
import ProductTable from "./productTable/ProductPhoneTable";
import ProductPhone from "./productTable/ProductPhone";
import ProductManWomenTable from "./productTable/ProductManWomenTable";
import ProductWatchTable from "./productTable/ProductWatchTable";
import ProductComment from "./ProductComment";
import ProductTvTable from "./productTable/ProductTvTable";
import ProductHeadPhone from "./productTable/ProductHeadPhone";
import addToCart from "../../redux/actions/cartActions";
import { openSnacbar } from "../../redux/actions/appActions";
export default function ProductDetails() {
  const { product } = useSelector((state) => state.product);
  const navigate = useNavigate();
  const dispacth = useDispatch();
  const handleToCart = (cartItem) => {
    dispacth(addToCart(cartItem));
  };
  const { id } = useParams();
  useEffect(() => {
    dispacth(getOneProductWithDetail(id));
  }, []);
  const handleAddToCart = (product) => {
    dispacth(addToCart(product));
    dispacth(
      openSnacbar({
        message: "Sepete Eklendi",
        severity: "success",
      })
    );
  };
  return (
    <Container sx={{ mt: 3 }} maxWidth="lg">
      <Grid container spacing={3}>
        <Grid item xs={12} md={4}>
          <Stack spacing={2}>
            <Typography variant="h8">
              <h4 style={{ textAlign: "center" }}>
                {product?.category?.categoryName} Kategorisi
              </h4>
            </Typography>

            <img src={`${product.productImage}`} alt={product?.productName} />
            <Typography variant="h6">{product?.productName}</Typography>
            <ButtonGroup variant="outlined">
              <Button variant="contained" onClick={() => navigate("/products/list")}>
                Go to Products List{" "}
              </Button>
            </ButtonGroup>
          </Stack>
        </Grid>
        <Grid item xs={12} md={8}>
          <Typography align="right" variant="h2">
            {product.categoryId === 1 && (
              <Button
                style={{
                  backgroundColor: "black",
                  color: "white",
                  alignItems: "stretch",
                }}
                onClick={() =>
                  navigate(
                    `/productbrand/list/${product?.productDetails?.pc?.brandId}`
                  )
                }
              >
                Markayı ziyaret et
              </Button>
            )}
            {product.categoryId === 2 && (
              <Button
                style={{ backgroundColor: "black", color: "white" }}
                onClick={() =>
                  navigate(
                    `/productbrand/list/${product?.productDetails?.watch?.brandId}`
                  )
                }
              >
                Markayı ziyaret et
              </Button>
            )}
            {product.categoryId === 3 && (
              <Button
                style={{ backgroundColor: "black", color: "white" }}
                onClick={() =>
                  navigate(
                    `/productbrand/list/${product?.productDetails?.tv?.brandId}`
                  )
                }
              >
                Markayı ziyaret et
              </Button>
            )}
            {product.categoryId === 4 ||
              (product.categoryId === 5 && (
                <Button
                  style={{ backgroundColor: "black", color: "white" }}
                  onClick={() =>
                    navigate(
                      `/productbrand/list/${product?.productDetails?.manWomen?.brandId}`
                    )
                  }
                >
                  Markayı ziyaret et
                </Button>
              ))}
            {product.categoryId === 6 && (
              <Button
                style={{ backgroundColor: "black", color: "white" }}
                onClick={() =>
                  navigate(
                    `/productbrand/list/${product?.productDetails?.phone?.brandId}`
                  )
                }
              >
                Markayı ziyaret et
              </Button>
            )}
            {product.categoryId === 7 && (
              <Button
                style={{ backgroundColor: "black", color: "white" }}
                onClick={() =>
                  navigate(
                    `/productbrand/list/${product?.productDetails?.headPhone?.brandId}`
                  )
                }
              >
                Markayı ziyaret et
              </Button>
            )}
            <Button
              style={{ backgroundColor: "black", color: "white" }}
              onClick={() =>
                navigate(
                  `/productcategory/list/${product?.category?.categoryId}`
                )
              }
            >
              Kategoriyi ziyaret et
            </Button>
          </Typography>

          {/* <Typography variant="h6">
            {product?.productDetails?.productExplanation}
          </Typography> */}

          <br></br>
          <Typography sx={{ color: "red" }} align="center" variant="h6">
            Özellikler
          </Typography>

          {product.categoryId === 1 && (
            <ProductTable
              productTable={product?.productDetails?.pc}
            ></ProductTable>
          )}
          {product.categoryId === 6 && (
            <ProductPhone
              productPhone={product?.productDetails?.phone}
            ></ProductPhone>
          )}
          {product.categoryId === 2 && (
            <ProductWatchTable
              productWatch={product?.productDetails?.watch}
            ></ProductWatchTable>
          )}
          {product.categoryId === 7 && (
            <ProductHeadPhone
              productHeadPhone={product?.productDetails?.headPhone}
            ></ProductHeadPhone>
          )}
          {product.categoryId === 3 && (
            <ProductTvTable
              productTvTable={product?.productDetails?.tv}
            ></ProductTvTable>
          )}
          {(product.categoryId === 5 || product.categoryId === 4) && (
            <ProductManWomenTable
              productManWomen={product?.productDetails?.manWomen}
            ></ProductManWomenTable>
          )}
          <br></br>
          <Typography align="center">
            <Button
              onClick={() => handleAddToCart(product.productId)}
              color="warning"
              variant="contained"
            >
              Sepete Ekle
            </Button>
          </Typography>

          <Typography sx={{ color: "#f00" }} align="center" variant="h5">
            <br></br>
            {product?.productPrice} TL
          </Typography>
        </Grid>
      </Grid>
      <Typography sx={{ mt: 5 }} variant="h6">
        {product?.productDetails?.productExplanation}
      </Typography>
      {(product.categoryId === 5 || product.categoryId === 4) && (
        <Container sx={{ mt: 10 }}>
          <img
            style={{
              width: 600,
              height: 600,
              margin: "auto",
              display: "block",
            }}
            src={product.productDetails?.manWomen.image1}
          ></img>
          <img
            style={{
              width: 600,
              height: 600,
              marginTop: 50,
              margin: "auto",
              display: "block",
            }}
            src={product.productDetails?.manWomen.image2}
          ></img>
          <img
            style={{
              width: 600,
              height: 600,
              marginTop: 50,
              margin: "auto",
              display: "block",
            }}
            src={product.productDetails?.manWomen.image3}
          ></img>
        </Container>
      )}
      {product.categoryId === 1 && (
        <Container sx={{ mt: 10 }}>
          <img
            style={{
              width: 600,
              height: 600,
              margin: "auto",
              display: "block",
            }}
            src={product.productDetails?.pc?.image1}
          ></img>
          <img
            style={{
              width: 600,
              height: 600,
              marginTop: 50,
              margin: "auto",
              display: "block",
            }}
            src={product.productDetails?.pc?.image2}
          ></img>
          <img
            style={{
              width: 600,
              height: 600,
              marginTop: 50,
              margin: "auto",
              display: "block",
            }}
            src={product.productDetails?.pc?.image3}
          ></img>
        </Container>
      )}
      {product.categoryId === 2 && (
        <Container sx={{ mt: 10 }}>
          <img
            style={{
              width: 600,
              height: 600,
              margin: "auto",
              display: "block",
            }}
            src={product.productDetails?.watch?.image1}
          ></img>
          <img
            style={{
              width: 600,
              height: 600,
              marginTop: 50,
              margin: "auto",
              display: "block",
            }}
            src={product.productDetails?.watch?.image2}
          ></img>
          <img
            style={{
              width: 600,
              height: 600,
              marginTop: 50,
              margin: "auto",
              display: "block",
            }}
            src={product.productDetails?.watch?.image3}
          ></img>
        </Container>
      )}
      {product.categoryId === 3 && (
        <Container sx={{ mt: 10 }}>
          <img
            style={{
              width: 600,
              height: 600,
              margin: "auto",
              display: "block",
            }}
            src={product.productDetails?.tv?.image1}
          ></img>
          <img
            style={{
              width: 600,
              height: 600,
              marginTop: 50,
              margin: "auto",
              display: "block",
            }}
            src={product.productDetails?.tv?.image2}
          ></img>
          <img
            style={{
              width: 600,
              height: 600,
              marginTop: 50,
              margin: "auto",
              display: "block",
            }}
            src={product.productDetails?.tv?.image3}
          ></img>
        </Container>
      )}
      {product.categoryId === 6 && (
        <Container sx={{ mt: 10 }}>
          <img
            style={{
              width: 600,
              height: 600,
              margin: "auto",
              display: "block",
            }}
            src={product.productDetails?.phone?.image1}
          ></img>
          <img
            style={{
              width: 600,
              height: 600,
              marginTop: 50,
              margin: "auto",
              display: "block",
            }}
            src={product.productDetails?.phone?.image2}
          ></img>
          <img
            style={{
              width: 600,
              height: 600,
              marginTop: 50,
              margin: "auto",
              display: "block",
            }}
            src={product.productDetails?.phone?.image3}
          ></img>
        </Container>
      )}{" "}
      {product.categoryId === 7 && (
        <Container sx={{ mt: 10 }}>
          <img
            style={{
              width: 600,
              height: 600,
              margin: "auto",
              display: "block",
            }}
            src={product.productDetails?.headPhone.image1}
          ></img>
          <img
            style={{
              width: 600,
              height: 600,
              marginTop: 50,
              margin: "auto",
              display: "block",
            }}
            src={product.productDetails?.headPhone.image2}
          ></img>
          <img
            style={{
              width: 600,
              height: 600,
              marginTop: 50,
              margin: "auto",
              display: "block",
            }}
            src={product.productDetails?.headPhone.image3}
          ></img>
        </Container>
      )}
      <hr></hr>
      <ProductComment key={product.comment?.commentId}></ProductComment>
      <h4 style={{ textAlign: "right" }}>
        {" "}
        <Button>
          <a href="#top">
            <img
              style={{ width: 50, height: 50 }}
              src="data:image/png;base64,"
            />
          </a>
        </Button>
      </h4>
    </Container>
  );
}
